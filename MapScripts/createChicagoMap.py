...
def createChicagoMap(price=200):
    community_map = gpd.read_file('SHP\Chicago_SHP\Original_Shp\geo_export_6bce7db1-9e88-4f7c-93ce-9fc323a89fe0.shp')
...
# Commenting out this section and saving file directly
    ...
    # geo_df.to_file("Chicago_SHP\Summarized_Shp\ChicagoCrime.shp")
    geo_df = gpd.read_file("SHP\Chicago_SHP\Summarized_Shp\ChicagoCrime.shp")
    ...
    myscale = (map_and_stats_2['count'].quantile((0,0.25,0.75,0.9,0.98,1))).tolist()
    ...
    df_airbnb = pd.read_csv('./Data/Chicago_listings.csv')
    ...
    mymap.save("./static/index_Chicago.html")
